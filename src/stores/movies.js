...
export const useMovieStore = defineStore('movie', {
    ...
    actions: {
        async fetchMovies() {
            try {
                // 1. 发送请求获取完整数据
                const response = await fetch("https://apis.netstart.cn/maoyan/index/mostExpected?ci=1&limit=100&offset=0");
                if (!response.ok) {
                ...
                const movieList = fullData.coming || [];
                ...
                const parsedMovies = movieList.map(movie => {
                    ...
                    return {
                        ...
                        genre: genres[Math.floor(Math.random() * 4)], // 类型（如果接口返回中有的话，这里假设字段名）
                        rating: movie.score || "暂无评分", // 评分（0分显示“暂无评分”）
                        releaseDate: movie.comingTitle, // 上映日期
                        // releaseDateText: movie.comingTitle, // 上映日期文字描述（如“7月5日 周六”）
                        actors: movie.actors || "", // 主演（假设接口返回中有 star 字段） 在新接口里类别，比如冒险，动作
